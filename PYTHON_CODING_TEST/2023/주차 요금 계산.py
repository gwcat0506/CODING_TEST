import math
def calculator(in_time,out_time):
...
#       출차
        if state == 'OUT':
            dic[number][1] = dic[number][1]+calculator(dic[number][0],h*60+m)
            dic[number][2] = 'OUT'
            continue
#       재입차
        if state == 'IN':
...
#     차량이 남아있을 경우 23:59분의 요금으로 계산
    for number in dic:
        if dic[number][2]=='IN':
            dic[number][1] = dic[number][1]+calculator(dic[number][0],23*60+59)
            dic[number][2] = 'OUT'
            
    result = []
    dic.sorted(ascending = True)
    for number in dic:
        result.append(dic[number][1])
